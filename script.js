...
function allTasks() {
    ...
    // if tasks' length is 0 then pending num text content will no, if not then pending num value will be tasks' length
...
inputField.addEventListener("keyup", (e) => {
    ...
    if (e.key === "Enter" && inputVal.length > 0) {
      let liTag = ` <li class="list pending" onclick = "handleStatus(this)">
        ...
        todoList.insertAdjacentHTML("beforeend", liTag);  //inserting li tag inside the todolist
        inputField.value = "";  //removing value from input field after added into list
        allTasks();
    }
});
